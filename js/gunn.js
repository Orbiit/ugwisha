...
/* PARSING */
  ...
  function identifyPeriod (name) {
    ...
    { return 'f' } else if (~name.indexOf('BRUNCH') || ~name.indexOf('BREAK')) return 'b'
  ...
  function parseEvents (events, dateObj) {
    ...
    let self; let alternate; let overrideSELF = false
    ...
    events.forEach(({ summary = '', description }) => {
      ...
      if (schedule !== undefined) {
        ...
        if (schedule || normalSchedules[weekDay].length) { alternate = schedule || [] }
      }
    })
    if (alternate) {
    ...
    Object.keys(schedules).forEach(day => {
      ...
      schedMonths[month] += schedules[day].map(({ period, start, end, selfGrades }) => {
        if (period.length !== 1) return ''
          ...
          return (period === 's' ? String.fromCharCode((selfGrades || defaultSelf) + selfCharOffset) : period) +
  ...
  function getSchedule (dateObj) {
    ...
    const dateName = dateObj.toISOString().slice(5, 10)
    const schedule = JSON.parse(JSON.stringify(scheduleData[dateName] || normalSchedules[dateObj.getUTCDay()]))
