...
function identifyPeriod (name) {
  ...
  if (name.includes('PERIOD')) {
    const letter = getPeriodRegex.exec(name)
...
function decodeStoredAlternates (string) {
  ...
  const schedules = {}
  for (let i = 0; i < dates[1].length; i += 2) { dates.push(dates[1].slice(i, i + 2) + oddDayEncoded) }
  for (let i = 0; i < dates[2].length; i += 2) { dates.push(dates[2].slice(i, i + 2) + evenDayEncoded) }
  for (let i = 0; i < dates[3].length; i += 3) { dates.push(dates[3].slice(i, i + 2) + `2dwgebgogt4gtjbljbk56kfmximxn7a${dates[3][i + 2]}nhoq`) }
  dates.slice(4).forEach(date => {
    ...
    date.replace(periodRegex, (m, period, start, end) => {
      ...
      if (period[0] === 'a') { pd.advisoryAudience = advisoryAudiences[period[1]] }
      periods.push(pd)
    })
...
function encodeStoredAlternates ({ lastGenerated, schedules }) {
  ...
  const advisoryAudiences = []
  let oddDays = ''; let evenDays = ''; let advisoryDays = ''
